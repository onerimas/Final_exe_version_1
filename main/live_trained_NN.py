...
while True:
    ...
    with tf.Session() as sess:
        ...
        if predicted_class == 0:
            predicted_class="YouTube,"
            myCmd = 'ip net exec ue_nas tc qdisc replace dev netc root tbf rate 1464Kbit latency 50ms burst 1540'
            os.system(myCmd)
            myCmd = 'ip net exec enb tc qdisc replace dev netc root tbf rate 1464kbit latency 50ms burst 1540'
            os.system(myCmd)
        elif predicted_class== 1:
            predicted_class="FaceBook,"
            myCmd = 'ip net exec ue_nas tc qdisc replace dev netc root tbf rate 1464kbit latency 50ms burst 1540'
            ...
            predicted_class="SkypeVoiceCall,"
            myCmd = 'ip net exec ue_nas tc qdisc replace dev netc root tbf rate 74kbit latency 50ms burst 1540'
            os.system(myCmd)
            myCmd = 'ip net exec enb tc qdisc replace dev netc root tbf rate 74kbit latency 50ms burst 1540'
            os.system(myCmd)
        elif predicted_class== 4:
            predicted_class="SkypeVideoCall,"
            myCmd = 'ip net exec ue_nas tc qdisc replace dev netc root tbf rate 488kbit latency 50ms burst 1540'
            os.system(myCmd)
            myCmd = 'ip net exec enb tc qdisc replace dev netc root tbf rate 488kbit latency 50ms burst 1540'
            os.system(myCmd)
        else:
            predicted_class="DO_NOT_KNOW"
